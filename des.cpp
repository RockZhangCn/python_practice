...
unsigned char * encrypt_buf_ = new unsigned char[1024];
struct des_ctx ctx_;
int encrypt_domain(const char * v, char ** outData, int outLen)
{
    memset(encrypt_buf_, 0, 1024);
...
int decrypt_result(const char * results, char ** outData, int outLen)
{
    ...
    int len = StrToUChar(results, buf);
    len = decrypt_str(&ctx_, (u8*)buf, len, (u8*)*outData);
    return len;
}
...
void test_method()
{
    ...
    des_setkey(&ctx_, (u8 *)key, strlen(key));

    char * encrypt = new char[1024];
    int len = encrypt_domain(v, &encrypt, 1024);
    printf("encrypted len is %d , data is %s\n", len, encrypt);

    char * rawData = new char[1024];
    int  ret = decrypt_result(encrypt, &rawData, 1024);
...
#if 0
int encrypt_domain(const char * v, unsigned char ** outData, int outLen)
{
    memset(encrypt_buf_, 0, 1024);
    int len = encrypt_str(&ctx_, (u8*)v, strlen(v), encrypt_buf_);
    //int ret = UCharToStr(encrypt_buf_, len, *outData);
